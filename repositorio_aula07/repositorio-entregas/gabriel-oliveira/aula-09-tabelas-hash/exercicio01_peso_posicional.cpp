...
int hashPeso(string texto) {
    ...
    for (int i = 0; i < (int)texto.size(); i++) {
        soma = soma + primo(texto[i]) * (i + 1);   // posicao comeca em 1
    }
...
int main() {
    string nomes[4] = {"Esther", "Ben", "Bob", "Dan"};

    for (string nome : nomes) {
        cout << nome << " " << hashPeso(nome) << endl;
    }

    // Nao houve colisoes entre Esther, Ben, Bob e Dan.
    // Com o primos.h fornecido, "Bob" e "boB" dao o mesmo indice,
    // pois primo() usa tolower().
